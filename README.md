...
| 应用商店 | 搜索和分类安装运行环境；内置常用 Docker 应用模板并支持一键部署、绑站与 SSL |
| 服务检测 | 识别 Nginx、Apache、Docker Engine/Compose、certbot（含 Snap）、acme.sh 的版本、状态与路径 |
| Docker | 容器生命周期、镜像/网络/卷清单与受限操作、自定义一键部署、Compose 任务、鉴权终端 |
| 文件管理 | 从根目录浏览服务器文件，支持文本编辑、新建、重命名和删除 |
...
首版聚焦单服务器、单管理员场景，不包含多服务器集群、数据库/PHP 应用商店、文件上传预览、宿主机 Web 终端或防火墙管理。尚未在完整发行版矩阵中验证的功能会明确记录在路线图中。
